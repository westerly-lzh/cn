...
**定义1**：若给定的概率分布 $P=(p_1,P_2,...,p_n)$,则由该分布传递的信息量称为P的熵，即：$$I(P) = -(p_1 \times log_2(p_1)+ p_2 \times log_2(p_2) + ... + p_n \times log_2(p_n))$$

**定义2**:若一个给定的记录集合T根据类别属性，被分成相互独立的类$C_1,C_2,...,C_k$,则识别T的一个记录所属的类别所需要的信息量是$info(T) = I(P) $,其中P表示$(C_1,C_2,...,C_k)$的概率分布，即：$P=(|C_1|/|T|,|C_2|/|T|,...,|C_k|/|T|)$

**定义3**:若先根据非类别属性X的值将T分成集合$T_1,T_2,...,T_n$，则确定T中一个记录所属类别的信息量可以通过确定$T_i$的加权平均得到，即：$$info(X,T) = \sum_{i=1}^n |T_i|/|T| \times info(T_i)$$
